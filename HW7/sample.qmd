...
## Introduction
The group is divided into two sub-topic pairs: RNA-Seq Aligners (Students 1 and 2) and DEG Analysis (Students 3, 4, and 5). All five students work with the same Arabidopsis RNA-seq dataset and share a common analysis entry point, but each student applies a distinct method or tool to answer their sub-topic’s core question. The aligner sub-topic benchmarks two quantification strategies on the same raw reads; the DEG sub-topic benchmarks three differential expression methods on the same count matrix. This design allows direct cross-student comparison of results within each sub-topic [@Zhang2008-pc].
